...
async function analyzeSPsFromCode(folderName, ignoreFolders = undefined) {
    ...
    for (const jsFile of jsFiles) {
        ...
        const regexPattern = new RegExp('llamarSP\\s*\\(\\s*([\'"])([^\'"]*)\\1\\s*.*?\\s*([\'"]([^\'"]*)[\'"])?\\s*\\)', 'g');
    ...
    Object.keys(usedScriptNames).forEach((conexion) => {
        ...
        usedScriptInConexion.forEach(sp => {
            if (!spsLowerCased.includes(sp) && !ignoredScriptList.includes(sp)) {

...
function normalizeText(text) {
  ...
  // text = text.replace(/CREATE\s+PROCEDURE\s+(?:\[dbo\]\.)?\[([^\]]+)\]/gi, function (match, spName) {
  ...
  const regex = /CREATE PROCEDURE (?:\[dbo\]\.|\bdbo\.)?\[?(\w+)\]?/i;
  text = text.replace(regex, 'CREATE PROCEDURE $1');

  text = text.replace(/(CREATE\s+PROCEDURE )(\w+)/i, (match, createProcedure, firstWord) => {
